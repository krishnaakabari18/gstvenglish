...
const onScroll = () => {
  ...
  // 🎧 Auto play audio when article visible
useEffect(() => {
  const handleScroll = () => {
...
const distance = Math.abs(rect.top + rect.height / 2 - center);
    ...
    if (closestIndex !== null) {
      setActiveAudioIndex(closestIndex);
    }
  };
        ...
        {newsItem.id && (
          <div className="news-audio-player" style={{ marginTop: "15px" }}>
            <audio
              ...
              preload="metadata"
              style={{ width: "100%" }}
              onPlay={() => setActiveAudioIndex(index)}
            >
              <source src={`https://staging.gstv.in/backend2/storage/app/public/news_audio/news_${newsItem.id}.mp3`} type="audio/mpeg" />
            </audio>
          </div>
        )}
