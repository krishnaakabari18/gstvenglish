...
import { fetchEpaperDetail, EpaperItem } from '@/services/epaperApi';
...
const EpaperDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    if (city && date) loadEpaperDetail(city, date);
  }, [city, date]);

  const loadEpaperDetail = async (citySlug: string, dateStr: string) => {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchEpaperDetail(citySlug, dateStr);
      if (!data) {
      ...
      setError('Failed to load epaper');
    } finally {
      setLoading(false);
    }
  };
...
const getPaginationPages = (current: number, total: number, windowSize = 6) => {
  if (total <= windowSize) {
    return Array.from({ length: total }, (_, i) => i + 1);
  }
    ...
    return (
      <div className="container-fluid epaper-detail-container">
        <div className={COMMON_CLASSES.LOADING_CONTAINER}>
          <LoadingSpinner
            message="ઈ-પેપર લોડ કરી રહ્યા છીએ..."
            size="large"
            color="#850E00"
          />
        </div>
        ...
        <div className="epaperDetailMain">
          <div className="container text-center p-5">
            <h2 className="custom-gujrati-font" style={{textAlign:'center'}}>
              આ તારીખ માટે ઈ-પેપર ઉપલબ્ધ નથી
            </h2>
            <button
  ...
  return (
    <>
      <Head>
        <title>{epaper.etitle} | GSTV</title>
      </Head>
          ...
          <div className="epapperPageDetail">
            ...
            <div className="epaperDTop epaperdetailpg">
              <div className="epaper-controls-left">
                <Link href="/epaper" className="epaperbackbtn">
              ...
              <div className="epaper-controls-right">
                <input
                  ...
                  onChange={(e) =>
                    router.push(`/epaper/${city}/${e.target.value.split('-').reverse().join('-')}`)
                  }
  ...
  preventClicks={false}                 // ✅ REQUIRED
  preventClicksPropagation={false}      // ✅ REQUIRED
...
>
                {epaper.Story_data.map((img, i) => (
                  <SwiperSlide key={i}>
                   <div
  ref={containerRef}
  className="epaper-image-container"
  onClick={handleImageClick}   // ✅ MOVE HERE
...
>
  ...
  {/* Page Numbers */}
  {getPages(currentPage, epaper.Story_data.length).map(p => (
    // <button
    //   key={p}
  ...
  <button
    className="page-num"
    disabled={currentPage === epaper.Story_data.length}
    onClick={() => goToPage(currentPage + 1)}
    style={{ opacity: currentPage === epaper.Story_data.length ? 0.4 : 1 }}
  >
     {">>"}
  </button>
...
export default EpaperDetailPage;
